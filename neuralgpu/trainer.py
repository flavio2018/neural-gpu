...
def define_flags():
    ...
    tf.compat.v1.flags.DEFINE_float("init_weight", 1.0, "Initial weights deviation.")
    tf.compat.v1.flags.DEFINE_float("max_grad_norm", 1.0, "Clip gradients to this norm.")
    tf.compat.v1.flags.DEFINE_float("cutoff", 1.2, "Cutoff at the gates.")
    tf.compat.v1.flags.DEFINE_float("cutoff_tanh", 0.0, "Cutoff at tanh.")
    tf.compat.v1.flags.DEFINE_float("pull", 0.0005, "Starting pull of the relaxations.")
    tf.compat.v1.flags.DEFINE_float("pull_incr", 1.2, "Increase pull by that much.")
    tf.compat.v1.flags.DEFINE_float("curriculum_bound", 0.15, "Move curriculum < this.")
    tf.compat.v1.flags.DEFINE_float("dropout", 0.15, "Dropout that much.")
    tf.compat.v1.flags.DEFINE_integer("max_steps", 0, "Quit after this many steps.")
    tf.compat.v1.flags.DEFINE_integer("batch_size", 32, "Batch size.")
    tf.compat.v1.flags.DEFINE_integer("low_batch_size", 16, "Low batch size.")
    tf.compat.v1.flags.DEFINE_integer("steps_per_epoch", 200, "Steps per epoch.")
    tf.compat.v1.flags.DEFINE_integer("nmaps", 24, "Number of floats in each cell.")
    tf.compat.v1.flags.DEFINE_integer("niclass", 33, "Number of classes (0 is padding).")
    tf.compat.v1.flags.DEFINE_integer("noclass", 33, "Number of classes (0 is padding).")
    tf.compat.v1.flags.DEFINE_integer("max_length", 41, "Maximum length.")
    tf.compat.v1.flags.DEFINE_integer("rx_step", 6, "Relax that many recursive steps.")
    tf.compat.v1.flags.DEFINE_integer("random_seed", 125459, "Random seed.")
    tf.compat.v1.flags.DEFINE_integer("time_till_ckpt", 30, "How many tests per checkpoint")
    tf.compat.v1.flags.DEFINE_integer("time_till_eval", 2, "Number of steps between evals")
    tf.compat.v1.flags.DEFINE_integer("nconvs", 2, "How many convolutions / 1 step.")
    tf.compat.v1.flags.DEFINE_integer("kw", 3, "Kernel width.")
    tf.compat.v1.flags.DEFINE_integer("kh", 3, "Kernel height.")
    tf.compat.v1.flags.DEFINE_integer("height", 4, "Height.")
    tf.compat.v1.flags.DEFINE_integer("forward_max", 401, "Maximum forward length.")
    tf.compat.v1.flags.DEFINE_integer("nprint", 0, "How many test examples to print out.")
    tf.compat.v1.flags.DEFINE_integer("mode", 0, "Mode: 0-train other-decode.")
    tf.compat.v1.flags.DEFINE_bool("animate", False, "Whether to produce an animation.")
    tf.compat.v1.flags.DEFINE_float("smooth_grad", 0.0, "Whether to avoid clipping gradient")
    tf.compat.v1.flags.DEFINE_float("smooth_grad_tanh", 0.0, "Whether to avoid clipping tanh gradient")
    tf.compat.v1.flags.DEFINE_string("task", "badd", "Which task are we learning?")
    tf.compat.v1.flags.DEFINE_string("train_dir", "/tmp/neural", "Directory to store models.")

    tf.compat.v1.flags.DEFINE_float("layer_scale", 1.0, "Number of layers to use")

    # Batchnorm:     0 = none
    #                2 = correct
    #                1 = not quite correct, because of how masking is done, but simpler.
    tf.compat.v1.flags.DEFINE_integer("do_batchnorm", 0, "Whether to use batch normalization.")

    tf.compat.v1.flags.DEFINE_bool("do_resnet", False, "Whether to use resnets.")

    tf.compat.v1.flags.DEFINE_bool("print_one", True, "Print one example each evaluation")

    # output layer: 0 = standard: output layer n on length-n inputs
    #               1 = alternate: output sum of first n layers on length-n inputs.
    tf.compat.v1.flags.DEFINE_integer("output_layer", 0, "Which layer to output.")
    ...
    tf.compat.v1.flags.DEFINE_integer("progressive_curriculum", 0, "Whether to use progressive curriculum.")
    tf.compat.v1.flags.DEFINE_bool("taskid", False, "Feed task id to algorithm in each layer")

    tf.compat.v1.flags.DEFINE_bool("always_large", False, "Perform the large test even when the model is inaccurate")
